-- Use the email_queue approach for all leave types (same as the working approval system)
-- This restores the exact working logic that inserts into email_queue instead of HTTP calls
...
BEGIN
  ...
  -- Prepare leave data for email template (same as working version + reason)
  ...
  BEGIN
    PERFORM log_trigger_execution(
      ...
      NULL,
      NULL,
      'Email queued successfully for processing. Type: ' || p_email_type
    );
  EXCEPTION WHEN OTHERS THEN
    NULL;
  END;
...
'Email notification function that uses email_queue table for all leave event types - submissions, approvals, rejections, and withdrawals. Based on the working approval email logic.';
